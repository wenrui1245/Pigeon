...
public:
	...
	TextLabelClass* AddMessage(const wchar_t* Name, int ID, const wchar_t* Message, int ColoPhoboshemeIdx, TextPrintType Style, int Timeout, bool SinglePlayer)
		{ JMP_THIS(0x5D3BA0); }
	...
	TextLabelClass* AddEdit(int ColoPhoboshemeIdx, TextPrintType Style, wchar_t* Message, char CursorCharacter = 0, int Width = 640)
		{ JMP_THIS(0x5D4210); }
	...
	void SetEditColor(int ColoPhoboshemeIdx)
		{ JMP_THIS(0x5D4410); }
	...
	void PrintMessage(const wchar_t* pMessage, int durationFrames = 0x96, int nColoPhoboshemeIndex = ColoPhobosheme::White, bool bSilent = false)
		{ this->AddMessage(nullptr, 0, pMessage, nColoPhoboshemeIndex, static_cast<TextPrintType>(0x4046), durationFrames, bSilent); }

	void PrintMessage(const wchar_t* pMessage, double durationMinutes, int nColoPhoboshemeIndex = ColoPhobosheme::White, bool bSilent = false)
		{ this->AddMessage(nullptr, 0, pMessage, nColoPhoboshemeIndex, static_cast<TextPrintType>(0x4046), static_cast<int>(durationMinutes * 900), bSilent); }
